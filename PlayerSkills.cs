...
internal class PlayerSkillsPatch
{
    ...
    // Returns every skill the game knows about.
    // The game data has no "Skills" resource folder (the game's own Resources.LoadAll("Skills") in PlayerSkills.Start returns an empty array), the skill prefabs are referenced from the player's progressionSkills list and the SkillTiers prefab that the skills menu loads.
    private static List<PlayerSkill> GetAllSkills()
    {
        var skills = new List<PlayerSkill>();

        if (Player.Instance && Player.Instance.skills)
        {
            foreach (var skill in Player.Instance.skills.progressionSkills.Where(skill => skill && !skills.Contains(skill)))
            {
                skills.Add(skill);
            }
        }

        var skillTiers = Resources.Load(nameof(SkillTiers)) as GameObject;
        if (!skillTiers) return skills;
        {
            var tiers = skillTiers.GetComponent<SkillTiers>();
            if (!tiers) return skills;
            foreach (var tier in tiers.tiers.Where(tier => tier != null))
            {
                foreach (var skill in tier.skills.Where(skill => skill && !skills.Contains(skill)))
                {
                    skills.Add(skill);
                }

                foreach (var skill in tier.negativeSkills.Where(skill => skill && !skills.Contains(skill)))
                {
                    skills.Add(skill);
                }
            }
    ...
    private static void LogAllAvailableSkills()
    {
        ...
        foreach (var skill in allSkills)
        {
            ...
            Plugin.Log.LogInfo($"  - {skillGameObject.name}: Manual={skill.isActivatedManually}, Status={status}, Config={configKey ?? "N/A"}");
        }
    }
    ...
    private static void AddSkillToLists(PlayerSkills skills, PlayerSkill skill)
    {
        ...
        if (skills.skills.Any(s => s && s.gameObject && s.gameObject.name == skillName))
        {
