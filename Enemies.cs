...
internal class EnemiesPatch
{
  ...
  {
    if (!Singleton<CharacterSpawner>.Instance) return;
  ...
  private static readonly Dictionary<NightScenario.CharacterToSpawn, int> NightBaseAmounts = new();
  ...
  public static bool NightEnemyMultiplierPrefix(CharacterSpawner __instance)
  {
    ...
    if (Mathf.Approximately(mult, 1f)) return true; // vanilla behavior
    ...
    foreach (var characterToSpawn in scenario.characters.Where(characterToSpawn => characterToSpawn != null))
    {
      if (!NightBaseAmounts.TryGetValue(characterToSpawn, out var baseAmount))
      {
        baseAmount = characterToSpawn.amount;
        NightBaseAmounts[characterToSpawn] = baseAmount;
      }
    ...
    if (!Player.Instance.whereAmI.bigLocation || !Player.Instance.whereAmI.bigLocation.playerBase)
    {
      return false;
    }
    ...
    {
      var candidates = scenario.characters.Where(characterToSpawn => characterToSpawn != null && !string.IsNullOrEmpty(characterToSpawn.characterName) && characterToSpawn.amount > 0 && characterToSpawn.spawned < characterToSpawn.amount).ToList();
      if (candidates.Count == 0) break;
      var pick = candidates[Random.Range(0, candidates.Count)];
      var character = __instance.spawnCharacterAround(Player.Instance.gameObject, Vector3.zero, 1500f, pick.characterName, true);
      ...
      if (chapterTwo && Random.Range(0f, 1f) > 0.5f)
      {
  ...
  private static readonly Dictionary<CharacterToSpawn, int> FreeRoamingBaseAmounts = new();
  ...
  private static class FreeRoamingMultiplierPatch
  {
    ...
    private static void Prefix(object __instance)
    {
      ...
      foreach (var characterToSpawn in chunk.biome.characters.Where(characterToSpawn => characterToSpawn is { useCharacterSpawnPoint: false }))
      {
        if (!FreeRoamingBaseAmounts.TryGetValue(characterToSpawn, out var baseAmount))
        {
          baseAmount = characterToSpawn.amount;
          FreeRoamingBaseAmounts[characterToSpawn] = baseAmount;
        }
        characterToSpawn.amount = (int)(baseAmount * mult);
      }
    }
  ...
  private static readonly Dictionary<CharacterToSpawn, int> GlobalBaseAmounts = new();
  ...
  {
    var mult = Plugin.CreatureEnemyMultiplierDay.Value;
    if (!Singleton<WorldGenerator>.Instance) return;
    var preset = Singleton<WorldGenerator>.Instance.getBiomePreset(__instance.type);
    if (!preset) return;
    foreach (var characterToSpawn in preset.characters.Where(characterToSpawn => characterToSpawn is { useCharacterSpawnPoint: true }))
    {
      if (!GlobalBaseAmounts.TryGetValue(characterToSpawn, out var baseAmount))
      {
        baseAmount = characterToSpawn.amount;
        GlobalBaseAmounts[characterToSpawn] = baseAmount;
      }
      characterToSpawn.amount = (int)(baseAmount * mult);
    }
  }
  ...
  public static void CharacterUpdatePostfix(Character __instance)
  {
    ...
    if (__instance.dummy || __instance.npc || __instance.invisible) return;
